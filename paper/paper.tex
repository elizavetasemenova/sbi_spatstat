...
biased, and increasingly so as preferentiality grows: at the strongest
$\gamma$ it overestimates the baseline intensity and the spatial variance
(overall bias $\PrefBiasNaiveBeta$ in $\beta_0$ and $\PrefBiasNaiveSig$ in
$\sigma$), and its credible intervals are miscalibrated (minimum $90\%$ coverage
$\PrefCovNaiveMin$, SBC $p=\PrefSbcNaive$). The design-aware NPE is essentially
unbiased across the whole range of $\gamma$ (bias $\PrefBiasPsBeta$ and
$\PrefBiasPsSig$), restores calibration (minimum coverage $\PrefCovPsMin$, SBC
$p=\PrefSbcPs$), and---as a bonus the ignorable analysis cannot provide---infers
the preferentiality itself ($\gamma$ recovered with $R^2=\GammaRtwo$;
Figure~\ref{fig:prefgamma}). A gold-standard NUTS sampler for the joint
preferential model confirms the design-aware posterior is correct, not merely
self-consistent: the two are indistinguishable (C2ST $\PrefCtst$ over
$\PrefNCompared$ datasets).
...
biased as sampling becomes more informative; the design-aware NPE (blue) stays
