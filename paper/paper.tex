...
prevalence survey (disease mapping)---the amortized estimators reproduce a NUTS
analysis (parameters and risk map) in milliseconds rather than minutes. Amortized SBI thus makes calibrated LGCP inference
...
Real malaria data (\ref{sec:malaria}) & disease mapping & NUTS & C2ST \MalCtst; risk map in \MalNpeMs\,ms \\
\bottomrule
\end{tabular}
\label{tab:overview}
\end{table}
...
The amortized posterior again matches gold-standard NUTS
(Figure~\ref{fig:malaria}): parameters overlap (C2ST $\MalCtst$; posterior means
$\beta_0\!=\!\MalBetaN$ vs.\ $\MalBetaM$, $\sigma\!=\!\MalSigN$ vs.\ $\MalSigM$,
$\ell\!=\!\MalEllN$ vs.\ $\MalEllM$), and the estimator is calibrated on held-out
data from this design (SBC $\min p=\MalSbc$). The amortized fit takes
$\MalNpeMs$~ms against $\MalNutsSec$~s for NUTS, and yields the same posterior
malaria-risk map. In an elimination setting, where the same survey is repeated
across seasons and districts, this is the difference between minutes and days of
compute.
