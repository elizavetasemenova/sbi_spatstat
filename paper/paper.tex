...
On a real crime point pattern the estimators---trained only on simulations---
reproduce a NUTS analysis (parameters and intensity map) with no retraining, in
seconds rather than minutes. Amortized SBI thus makes calibrated LGCP inference
...
(Section~\ref{sec:agg}).
\item Finally, we apply the estimators---trained \emph{only} on simulations---to
a real crime point pattern with no retraining, and show they reproduce a
gold-standard NUTS analysis (parameters and intensity map) in seconds rather than
minutes (Section~\ref{sec:real}).
\end{enumerate}
...
\begin{table}[t]
\centering\small
\caption{Overview of experiments. Four of the seven are validated head-to-head
against a gold-standard No-U-Turn Sampler (NUTS).}
\begin{tabular}{@{}llll@{}}
\toprule
...
Intensity surface (\ref{sec:field}) & latent field & NUTS & map corr.\ \FieldMcmcMeanCorr; aggregates calibrated \\
...
Real data (\ref{sec:real}) & params + field & NUTS & zero-shot; map corr.\ \RealFieldCorr \\
\bottomrule
\end{tabular}
\label{tab:overview}
\end{table}
...
\section{A real-data case study}
\label{sec:real}

Everything so far is simulated, which is what makes calibration and gold-standard
comparison possible. We close with a real point pattern to show the pipeline
transfers. We use the \texttt{crimes} pattern from the geodanet example dataset distributed
with the PySAL library \citep{rey2007pysal}: $\RealN$ recorded crime locations in
a road-network neighbourhood, binned to the same $\Grid\times\Grid$ grid. Crucially we
\emph{do not retrain}: the very same NPE and field U-Net used throughout, trained
only on synthetic LGCPs, are applied to the real grid in a single forward pass
each---the amortization dividend in its purest form.
