...
high-dimensional latent Gaussian field. We show that \emph{amortized}
simulation-based inference (SBI) removes this per-dataset cost while preserving
the full Bayesian posterior. We train a neural posterior estimator---a
...
range) and attains near-nominal credible-interval coverage, whereas the common
point-estimate ``regression'' approach is severely overconfident. Against a
...
training cost after only \BreakEven{} datasets. Finally, an honest kernel
misspecification study shows the method degrades gracefully rather than failing
silently. Amortized SBI thus makes calibrated, likelihood-free LGCP inference
...
\citep{cranmer2020frontier}. One trains a neural conditional density estimator
$q_\phi(\thetavec\mid\xvec)$ on pairs $(\thetavec,\xvec)$ drawn from the prior and
the simulator, so that a single forward pass returns an approximate posterior for
any new dataset $\xvec$. The cost of inference is moved \emph{up front} and
\emph{shared} across all future datasets, and the method never evaluates the
likelihood---only simulates from it---so it extends naturally to observation
processes (thinning, preferential sampling, aggregation) that break analytic
approaches. The question this paper answers is whether such amortized posteriors
are \emph{trustworthy} for LGCPs: are they calibrated, and do they match what a
careful MCMC analysis would have returned?

\paragraph{Contributions.}
\begin{enumerate}\itemsep2pt
\item We cast Bayesian parameter inference for grid-discretized LGCPs as amortized
neural posterior estimation (NPE), pairing a convolutional summary network for the
count field with a masked autoregressive flow over the three interpretable model
parameters (Section~\ref{sec:method}).
\item We subject the learned posterior to the strongest calibration diagnostics in
the SBI toolkit---simulation-based calibration and credible-interval coverage---and
show it passes on all parameters, while the widely used point-estimate regressor is
badly miscalibrated (Section~\ref{sec:calibration}).
\item We validate against a gold-standard No-U-Turn Sampler: the amortized
posteriors are statistically indistinguishable from MCMC (classifier two-sample
accuracy \CtstMean{}) at a $\sim\!\Speedup\times$ lower per-dataset cost, breaking
even after \BreakEven{} datasets (Section~\ref{sec:mcmc}).
...
uniform. Figure~\ref{fig:sbc} shows the rank empirical CDFs for all three
parameters lying inside the $95\%$ uniformity band, with $\chi^2$ uniformity
$p$-values all above \SbcPmin{}: the amortized posterior is calibrated in the
strong distributional sense SBC demands.

\paragraph{Coverage.}
Figure~\ref{fig:coverage} plots empirical against nominal central-interval
coverage. The NPE (CNN$+$MAF) tracks the diagonal closely---$90\%$ intervals
cover $\CovNpeB/\CovNpeS/\CovNpeL$ for $\beta_0/\sigma/\ell$---as does the
hand-crafted-summary NPE. The point-estimate regressor, by contrast, is severely
overconfident: its nominal $90\%$ intervals cover only
$\CovRegB/\CovRegS/\CovRegL$. This is the central practical warning of the paper:
inverting a simulator with a regression network yields point predictions with no
usable uncertainty, whereas NPE yields a calibrated posterior at essentially the
same training cost.
...
\caption{Simulation-based calibration. Rank ECDFs (blue) for the three parameters
fall within the $95\%$ uniformity band (grey); the diagonal is perfect
calibration. Deviations would indicate bias (S-shape) or over/under-dispersion.}
\label{fig:sbc}
\end{figure}
...
\caption{Credible-interval calibration. The NPE methods track the diagonal;
the point-estimate regressor (orange) is markedly overconfident, its intervals
covering far less than nominal.}
\label{fig:coverage}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Held-out accuracy and calibration ($\NTest$ datasets). NPE variants are
calibrated (coverage $\approx0.90$); the regressor is overconfident.}
...
\paragraph{SBI for spatial and point-process models.}
Amortized inference has begun to reach spatial statistics and ecology, including
neural estimators for spatial covariance parameters and point processes
\citep{gerber2021fast,lenzi2023neural,sainsbury2024likelihood}. We add to this
line a focused, rigorously validated treatment of the LGCP that (i) delivers a
\emph{full calibrated posterior} rather than a point estimate, and (ii)
benchmarks it head-to-head against gold-standard MCMC in both fidelity and cost.
...
Our study fixes the grid resolution and the kernel family, infers three global
parameters, and does not reconstruct the latent field; the misspecification
analysis shows real sensitivity to smoothness. Natural extensions are to amortize
over grid resolution and domain shape, to jointly infer the latent intensity
surface (a spatial prediction task), to add covariates and non-stationarity, and
to handle realistic observation processes---thinning, preferential sampling and
aggregation---where the likelihood-free nature of SBI is decisive and analytic
competitors do not apply. Treating the kernel smoothness as an inferred parameter,
rather than a fixed modelling choice, is a promising route to robustness. We view
