"""Grid-count baseline: identical simulator/pipeline, but the conditioning
features are a 20x20 binned-count grid instead of grid-free spectral features.
This is the apples-to-apples ablation for the correlation-range (ell) recovery
claim -- same targets, same flow-matching sampler, only the summary changes."""
...
def main():
    ...
    model.eval()
    S = model.sample(torch.tensor(te["phi"]), n_per=400, steps=80).numpy()
    th_post = priors.from_unit(S[:, :, :3].reshape(-1, 3)).reshape(N_TEST, 400, 3)
    ...
    for p, nm in enumerate(["beta0", "sigma", "ell"]):
        ...
        cov = float(((th_true[:, p] >= lo) & (th_true[:, p] <= hi)).mean())
    ...
    # field recovery from the baseline coefficients
